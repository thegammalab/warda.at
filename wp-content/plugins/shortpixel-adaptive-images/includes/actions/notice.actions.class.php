	...
	class Actions {
		...
		public static function handle() {
			...
			if ( !empty( $causer ) && !empty( $data ) ) {
				$response = call_user_func( [ 'self', 'handle' . Converter::toTitleCase( $causer ) ], $data, $ctrl );
			}
		...
		private static function handleWpRocketLazy( $data ) {
			...
			if ( $action === 'dismiss' ) {
				return [
					'success' => Notice::dismiss( 'wp rocket lazy' ),
				];
			}
		...
		private static function handleWpRocketCss( $data ) {
			...
			if ( $action === 'dismiss' ) {
				return [
					'success' => Notice::dismiss( 'wp rocket css' ),
				];
			}
		...
		private static function handleKey( $data, $controller ) {
			...
			if ( $action === 'use account' ) {
				$success = $controller->use_shortpixel_account();
		...
		private static function handleBeta( $data ) {
			...
			if ( $action === 'dismiss' ) {
				return [
					'success' => Notice::dismiss( 'beta', SHORTPIXEL_AI_VERSION ),
				];
			}
