<?php
	...
	class Feedback {
		...
		public static function send( $reason, $suggestions = null, $anonymous = false ) {
			...
			if ( !$anonymous ) {
				$wordpress[ 'deactivated_plugin' ][ 'uninstall_details' ] .= ( empty( $wordpress[ 'deactivated_plugin' ][ 'uninstall_details' ] ) ? '' : PHP_EOL . PHP_EOL ) . 'Domain: ' . ShortPixelAI::_()->get_site_domain();
			}
			...
			if ( !empty( $spai_key ) && !!ShortPixelAI::_()->get_cdn_domain_usage( null, $spai_key ) && !$anonymous ) {
				$body[ 'key' ] = $spai_key;
			}

			return Request::post( ShortPixelAI::SP_API . 'v2/feedback.php', [ 'body' => $body ] );
		}
		...
		private static function collectUserData( $anonymous ) {
			...
			if ( $user && !$anonymous ) {
				...
				$return[ 'domain' ]     = ShortPixelAI::_()->get_site_domain();
			}
