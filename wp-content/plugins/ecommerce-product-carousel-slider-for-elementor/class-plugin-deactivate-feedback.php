...
if( ! class_exists( 'WPCE_Usage_Feedback') ) {
	
	class WPCE_Usage_Feedback {
		
		private $WPCE_VERSION = '0.9.0';
		private $home_url = '';
		private $plugin_file = '';
		private $plugin_name = '';
		...
		public function init() {
			global $pagenow;
				...
				add_action( 'wp_ajax_wpce_goodbye_form', array( $this, 'goodbye_form_callback' ) );
		...
		public function admin_enqueue_scripts(){
			wp_enqueue_style('wbps-admin-style', WPCE_URL . 'admin/assets/css/admin.css', array(), WPCE_VERSION, 'all');

			$css='';
			$css .= '.button.wpce-promo-link {
		      color: #ff0000;
		      font-weight: normal;
		      margin-left: 0;
		...
		public function send_data( $body ) {
			...
			foreach($body as $key=>$value){
			    ...
			    $title   = esc_html('Elementor WooCommerce Product Carousel Slider Free Plugin Deactivation Notice');
				$headers = array('From: Elementor WooCommerce Product Carousel Slider Free <mailer@just-a-fake-from-address.com>');
		...
		public function deactivate_this_plugin() {
			...
			if( false !== get_option( 'wpce_deactivation_reason_' . $this->plugin_name ) ) {
				$body['deactivation_reason'] = get_option( 'wpce_deactivation_reason_' . $this->plugin_name );
				delete_option('wpce_deactivation_reason_' . $this->plugin_name);
			}
			if( false !== get_option( 'wpce_deactivation_details_' . $this->plugin_name ) ) {
				$body['deactivation_details'] = get_option( 'wpce_deactivation_details_' . $this->plugin_name );
				delete_option('wpce_deactivation_details_' . $this->plugin_name);
			}

			if( false !== get_option( 'wpce_deactivation_email_' . $this->plugin_name ) ) {
				$body['deactivation_email'] = get_option( 'wpce_deactivation_email_' . $this->plugin_name );
				delete_option('wpce_deactivation_email_' . $this->plugin_name);
			}

			if( false !== get_option( 'wpce_deactivation_main_reason_' . $this->plugin_name ) ) {
				$body['deactivation_main_reason'] = get_option( 'wpce_deactivation_main_reason_' . $this->plugin_name );
				delete_option('wpce_deactivation_main_reason_' . $this->plugin_name);
			}
		...
		public function filter_action_links( $links ) {
			...
			if( isset( $links['deactivate'] ) && $this->include_goodbye_form ) {
				...
				$deactivation_link = str_replace( '<a ', '<div class="wpb-goodbye-form-wrapper"><span class="wpb-goodbye-form" id="wpb-goodbye-form-' . esc_attr( $this->plugin_name ) . '"></span></div><a onclick="javascript:event.preventDefault();" id="wpb-goodbye-link-wpce-' . esc_attr( $this->plugin_name ) . '" ', $deactivation_link );
		...
		public function form_filterable_text() {
			...
			return apply_filters( 'wpce_form_text_' . esc_attr( $this->plugin_name ), $form );
		}
		...
		public function goodbye_ajax() {
			...
			if( is_array( $form['options'] ) ) {
				$html .= '<div class="wpce-goodbye-options"><p>';
				foreach( $form['options'] as $option ) {
					$html .= '<input type="radio" name="wpce-goodbye-options" id="' . str_replace( " ", "", esc_attr( $option ) ) . '" value="' . esc_attr( $option ) . '"> <label for="' . str_replace( " ", "", esc_attr( $option ) ) . '">' . esc_attr( $option ) . '</label><br>';
				}
				$html .= '</p><div id="wpce_additional_content" style="display:none;"><label for="wpce-goodbye-reasons">' . esc_html( $form['email'] ) .'</label><br><input type="email" name="wpce-goodbye-email" id="wpce-goodbye-email" value="'.get_option('admin_email').'" /> (Optional)';
				
				$html .= '<br><label for="wpce-goodbye-reasons">' . esc_html( $form['details'] ) .'</label><textarea name="wpce-goodbye-reasons" id="wpce-goodbye-reasons" rows="2" style="width:100%"></textarea></div>';
				$html .= '</div><!-- .wpce-goodbye-options -->';
			}
			...
			?>
			<div class="wpce-goodbye-form-bg"></div>


			<script>
				jQuery(document).ready(function($){
					 $(document).on('change', '#wpb-goodbye-form-woo-product-slider-for-elementor input[type=radio]', function() { 
						if($(this).val()=='Deactivating Temporarily' || $(this).val()=='Upgrading to Pro'){
							$('#wpce_additional_content').hide();
						}else{
							$('#wpce_additional_content').show();
						}
						
					 });

					$('#wpb-goodbye-link-wpce-<?php echo esc_attr( $this->plugin_name ); ?>').on('click',function(){
						var url = document.getElementById('wpb-goodbye-link-wpce-<?php echo esc_attr( $this->plugin_name ); ?>');
						...
						$('#wpb-submit-form').on('click', function(e){
							...
							$.each($('input[name=\'wpce-goodbye-options[]\']:checked'), function(){
								values.push($(this).val());
							});
							var email = $('#wpce-goodbye-email').val();
							var details = $('#wpce-goodbye-reasons').val();
							var deactivate_main_reason = $('input[name="wpce-goodbye-options"]:checked').val();
							var data = {
								'action': 'wpce_goodbye_form',
								'values': values,
								'main_reason': deactivate_main_reason,
								'details': details,
								'email': email,
								'security': '<?php echo wp_create_nonce ( 'wpce_goodbye_form' ); ?>',
								'dataType': 'json'
							}
						...
						$('.wpce-goodbye-form-bg').on('click',function(){
		...
		public function goodbye_form_callback() {
			//check_ajax_referer( 'wpce_goodbye_form', 'security' );
			if( isset( $_POST['values'] ) ) {
				$values = json_encode( wp_unslash( $_POST['values'] ) );
				update_option( 'wpce_deactivation_reason_' . $this->plugin_name, $values );
			}
			if( isset( $_POST['details'] ) ) {
				$details = sanitize_text_field( $_POST['details'] );
				update_option( 'wpce_deactivation_details_' . $this->plugin_name, $details );
			}

			if( isset( $_POST['email'] ) ) {
				$email = sanitize_email( $_POST['email'] );
				update_option( 'wpce_deactivation_email_' . $this->plugin_name, $email );
			}

			if( isset( $_POST['main_reason'] ) ) {
				$main_reason = sanitize_text_field( $_POST['main_reason'] );
				update_option( 'wpce_deactivation_main_reason_' . $this->plugin_name, $main_reason );
			}
